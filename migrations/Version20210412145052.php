 ...
 */
final class Version20210412145052 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__zakladni_test AS SELECT id, status, result FROM zakladni_test');
        $this->addSql('DROP TABLE zakladni_test');
        $this->addSql('CREATE TABLE zakladni_test (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, result INTEGER NOT NULL, status INTEGER NOT NULL)');
        $this->addSql('INSERT INTO zakladni_test (id, status, result) SELECT id, status, result FROM __temp__zakladni_test');
        $this->addSql('DROP TABLE __temp__zakladni_test');
    }
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__zakladni_test AS SELECT id, status, result FROM zakladni_test');
        $this->addSql('DROP TABLE zakladni_test');
        $this->addSql('CREATE TABLE zakladni_test (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, result INTEGER NOT NULL, status VARCHAR(3) NOT NULL COLLATE BINARY)');
        $this->addSql('INSERT INTO zakladni_test (id, status, result) SELECT id, status, result FROM __temp__zakladni_test');
        $this->addSql('DROP TABLE __temp__zakladni_test');
    }
}
